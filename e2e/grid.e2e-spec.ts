...
describe('Grid', () => {
  ...
  it('should match previous screenshot with checklist enabled', (done) => {
    SkyHostBrowser.get('visual/grid');
    SkyHostBrowser.setWindowBreakpoint('lg');
    SkyHostBrowser.scrollTo('#screenshot-grid-checklist');
    expect('#screenshot-grid-checklist').toMatchBaselineScreenshot(done);
  });

  it('should match previous screenshot with checklist enabled (screen: xs)', (done) => {
    SkyHostBrowser.get('visual/grid');
    SkyHostBrowser.setWindowBreakpoint('xs');
    SkyHostBrowser.scrollTo('#screenshot-grid-checklist');
    expect('#screenshot-grid-checklist').toMatchBaselineScreenshot(done);
  ...
  it('should match previous screenshot with selected rows', (done) => {
    ...
    SkyHostBrowser.setWindowBreakpoint('lg');
    SkyHostBrowser.scrollTo('#screenshot-grid-checklist');
    let checkboxes = element.all(by.css('.sky-grid-checklist-cell sky-checkbox'));
